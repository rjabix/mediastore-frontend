export default function CategoriesShortHList(){
    const products = [
        ...
        { name: 'Computers', image: 'https://via.placeholder.com/100' },
            ...
            <div className="container mx-auto px-4">
                <div className="flex justify-around items-center space-x-8">
                    {products.map((product, index) => (
                        <a href={'/categories/' + product.name.toLowerCase()} key={index} className="group text-center hover:scale-110 transition-transform duration-300">
                            <img src={product.image} alt={product.name}
                                 className="mx-auto mb-2 w-24 h-24 object-contain"
                                style = {{objectFit: "contain", width: "100px", height: "100px"}}/>
                            <p className="text-lg font-medium text-gray-800 group-hover:underline">{product.name}</p>
                        </a>
                    ))}
                </div>
            </div>
